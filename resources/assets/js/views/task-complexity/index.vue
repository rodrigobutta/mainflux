    ...
    <div>
        <div class="row page-titles">
            <div class="col-md-6 col-8 align-self-center">
                <h3 class="text-themecolor m-b-0 m-t-0">{{trans('task-complexity.task_complexity')}}</h3>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><router-link to="/home">{{trans('general.home')}}</router-link></li>
                    <li class="breadcrumb-item active">{{trans('task-complexity.task_complexity')}}</li>
                </ol>
            </div>
        </div>
        <div class="row">
            <div class="col-12 col-sm-4 col-md-4" v-if="hasPermission('create-task-complexity')">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">{{trans('task-complexity.add_new_task_complexity')}}</h4>
            ...
            <div class="col-12 col-sm-8 col-md-8" v-if="hasPermission('list-task-complexity')">
                                ...
                                <div class="col-6 col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="order" class="form-control" v-model="filterTaskComplexityForm.sortBy">
                                            <option value="name">{{trans('task-complexity.name')}}</option>
                                            <option value="description">{{trans('task-complexity.description')}}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-6 col-md-4">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <h4 class="card-title">{{trans('task-complexity.task_complexity_list')}}</h4>
                        ...
                        <div class="table-responsive" v-if="taskComplexitys.total">
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th>{{trans('task-complexity.name')}}</th>
                                        <th>{{trans('task-complexity.code')}}</th>
                                        <th>{{trans('task-complexity.description')}}</th>
                                ...
                                <tbody>
                                    <tr v-for="task_complexity in taskComplexitys.data" v-bind:key="task_complexity.id">
                                        ...
                                        <td class="table-option">
                                            <div class="btn-group">
                                                <button class="btn btn-info btn-sm" v-if="hasPermission('edit-task-complexity')" v-tooltip="trans('task-complexity.edit_task_complexity')" @click.prevent="editContractor(task_complexity)"><i class="fas fa-pencil-alt"></i></button>
                                                <button class="btn btn-danger btn-sm" v-if="hasPermission('delete-task-complexity')" :key="task_complexity.id" v-confirm="{ok: confirmDelete(task_complexity)}" v-tooltip="trans('task-complexity.delete_task_complexity')"><i class="fas fa-trash"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
