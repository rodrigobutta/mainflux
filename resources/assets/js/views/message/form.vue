<template>
    <form @submit.prevent="submit" @keydown="messageForm.errors.clear($event.target.name)">
        <div class="form-group">
            <v-select label="name" v-model="selected_user" name="to_user_id" id="to_user_id" :options="users" :placeholder="trans('message.select_recipient')" @select="onUserSelect" @close="messageForm.errors.clear('to_user_id')" @remove="messageForm.to_user_id = ''"></v-select>
        ...
        <div class="form-group pull-right">
            <button type="button" @click="saveAsDraft" class="btn btn-info waves-effect waves-light"><i class="fas fa-edit"></i> {{trans('message.save_as_draft')}}</button>
            <button v-if="messageForm.id" type="button" class="btn btn-danger waves-effect waves-light" v-confirm="{ok: confirmDelete(messageForm.uuid)}" v-tooltip="trans('message.delete',{type: trans('message.draft')})"><i class="fas fa-trash"></i> {{trans('message.delete',{type: trans('message.draft')})}}</button>
            <router-link to="/message/draft" v-if="messageForm.id" class="btn btn-warning waves-effect waves-light"><i class="fas fa-times"></i> {{trans('message.cancel')}}</router-link>
